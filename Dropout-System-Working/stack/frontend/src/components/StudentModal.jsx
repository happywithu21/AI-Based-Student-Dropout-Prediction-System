    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-2 sm:p-4 z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[95vh] sm:max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 sm:p-6 space-y-6 sm:space-y-8">
          ...
          {/* LPU Subjects */}
          {safeStudent.Recent_Subjects && safeStudent.Recent_Subjects.length > 0 && (
            <div>
              <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
                <GraduationCap className="w-4 h-4 sm:w-5 sm:h-5 text-indigo-600" />
                Current Registered Courses (LPU)
              </h3>
              <div className="bg-indigo-50 border border-indigo-100 rounded-lg p-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {safeStudent.Recent_Subjects.map((subject, index) => (
                    <div key={index} className="flex items-center gap-2 text-sm text-indigo-900 bg-white/50 p-2 rounded border border-indigo-200">

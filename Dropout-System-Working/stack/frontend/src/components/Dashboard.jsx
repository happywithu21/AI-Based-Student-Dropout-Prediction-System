...
const Dashboard = ({ students = [] }) => {
  ...
  // Get priority students (highest urgency score)
  const getPriorityStudents = (studentData, limit) => {
    return studentData
      .map(student => ({
        ...student,
        urgency_score: (
          ((student.Attendance_Percentage || 0) < 60 ? 3 : 0) +
          ((student.Avg_Test_Score || 0) < 40 ? 3 : 0) +
          ((student.Subjects_Failed || 0) >= 2 ? 2 : 0) +
          ((student.Fee_Due_Days || 0) > 60 ? 2 : 0) +
          ((student.Total_Risk_Flags || 0) * 0.5)
        )
      }))

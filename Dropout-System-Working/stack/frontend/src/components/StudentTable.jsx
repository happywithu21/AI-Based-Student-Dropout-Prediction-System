              ...
              <tr key={student.Student_ID} className="hover:bg-gray-50">
                ...
                <td className="px-3 py-2 whitespace-nowrap">
                  <div className="flex items-center">
                    ...
                    {student.Attendance_Percentage < 75 && (
                      <div className="ml-1 w-1.5 h-1.5 bg-red-400 rounded-full"></div>
                    )}
                  </div>
                </td>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="text-xs text-gray-600 max-w-[200px] truncate">
                    {student.Recent_Subjects && student.Recent_Subjects.length > 0 
                      ? student.Recent_Subjects[0] + (student.Recent_Subjects.length > 1 ? ` (+${student.Recent_Subjects.length - 1} more)` : '')
                      : 'None'}
                  </div>
                </td>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium min-w-[220px]">
                  <div className="flex items-center space-x-2 pr-4">
                    <button
                      onClick={() => onStudentSelect(student)}
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 text-blue-600 border border-blue-200 rounded-md hover:bg-blue-100 transition-colors"
                      title="View Details"
                    >
                    ...
                    {student.dropout_risk > 0 && (
                      <button
                        onClick={() => handleNotify(student.Student_ID, student.Name)}
                        disabled={notifying[student.Student_ID]}
                        className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-orange-50 text-orange-600 border border-orange-200 rounded-md hover:bg-orange-100 transition-colors disabled:opacity-50"
                        title="Send Alert"
                      >
                        <Bell className="w-4 h-4" />
                        <span>
                          {notifying[student.Student_ID] ? 'Wait...' : 'Alert'}
                        </span>
                      </button>
                    )}
                  </div>

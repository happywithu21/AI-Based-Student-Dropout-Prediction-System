...
const TrendAnalysis = ({ students = [] }) => {
  ...
  const trendData = useMemo(() => {
    ...
    for (let i = weeks; i >= 0; i--) {
      ...
      const avgAttendance = weekStudents.length > 0 
        ? weekStudents.reduce((sum, s) => sum + (s.Attendance_Percentage || 0), 0) / weekStudents.length
        : 0;
        
      const avgPerformance = weekStudents.length > 0
        ? weekStudents.reduce((sum, s) => sum + (s.Avg_Test_Score || 0), 0) / weekStudents.length
        : 0;
    ...
    if (recentAttendance.length >= 3) {
      ...
      if (isDecline && recentAttendance[2] < 80) {
        detected.push({
          ...
          affectedStudents: students.filter(s => s.Attendance_Percentage < 80).length
        });
      }
    }
    ...
    if (recentPerformance.length >= 2) {
      ...
      if (dropPercent > 10) {
        detected.push({
          ...
          affectedStudents: students.filter(s => s.Avg_Test_Score < 65).length
        });
      }
    }
  ...
  const predictions = useMemo(() => {
    ...
    students.forEach(student => {
      ...
      // Attendance trend prediction (Monthly vs Overall)
      const attTrend = (student.Monthly_Attendance || 0) - (student.Attendance_Percentage || 0);
      if (student.Attendance_Percentage < 85 && attTrend < -2) {
        riskFactors.push('Declining attendance trend');
        predictedRisk += 0.35;
      }

      // Performance trend (Last test vs Average)
      const perfTrend = (student.Last_Test_Score || 0) - (student.Avg_Test_Score || 0);
      if (student.Avg_Test_Score < 70 && perfTrend < -5) {
        riskFactors.push('Declining academic performance');
        predictedRisk += 0.3;
      }

      // Financial stress indicator
      if (student.Fee_Due_Days > 15) {
        riskFactors.push('Financial difficulties');
        predictedRisk += 0.2;
      }

      // Behavioral indicators (simulated based on failed subjects)
      if (student.Subjects_Failed > 1) {
      ...
      if (predictedRisk > 0.4) {
        predictions.push({
          ...
          timeframe: predictedRisk > 0.7 ? '2-3 weeks' : '4-6 weeks',
          riskFactors,
          confidence: Math.round((predictedRisk * 100) + 10)
        });
      }
    });
